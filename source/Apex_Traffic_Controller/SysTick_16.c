...
void SysTick_Init(void)
{
  ...
  NVIC_ST_CTRL_R = 0x00;                                                                        // p. 133 disable SysTick during setup  
        NVIC_ST_RELOAD_R = 0x682A;                                                      // p.135, clock is 16MHz, Maximum is 0x00FFFFFF = 16,777,215 (dec) 682A
        NVIC_ST_CURRENT_R = 0;                                  // any write to current clears it
  NVIC_ST_CTRL_R |= 0x07;                                       //  p. 133 enable SysTick , system clock as clock source, enable sysTick interrupt
 }
...
void SysTick_Wait10ms(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
    SysTick_Wait(500000);  // wait 10ms (assumes 16 MHz clock), 1 Systick is equal to 62.5nS, so to get 10ms, we need ( 1/62.5nS)*10mS =160000
  }
}
// Time delay using busy wait.
// This assumes 16 MHz system clock.
void SysTick_Wait1ms(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
    SysTick_Wait(50000);   // wait 1ms (assumes 16 MHz clock) 1 Systick is equal to 62.5nS, so to get 1ms, we need ( 1/62.5nS)*1mS =16000
  }
}
